...
Local Laptop (Terminal): Run gcert to get your SKMS/gnubby credentials.

Local Laptop (Terminal): Run code . to open VS Code. This creates a parent-child relationship so VS Code inherits your live LOAS cert in /tmp.

Cloudtop VS Code Terminal: Run gcert to ensure the remote machine sees your forwarded agent.

Cloudtop VS Code Terminal: Set up Application Default Credentials (ADC) for the Cloud layer (Vertex AI):
...
If package installations fail (401 Airlock errors): Run glogin to refresh OAuth2 tokens for internal tools like gpkg.

(Important: Projects are periodically deleted. If you suddenly have API issues, check if you need to remove the project from pending deletion and re-enable billing).
...
Create the .env file based on your project configuration:

env
GOOGLE_CLOUD_PROJECT_ID="blt-test-project-1"
GOOGLE_CLOUD_LOCATION="us-central1"
GOOGLE_GENAI_USE_VERTEXAI="true"
Initialize and install dependencies (Auto-Syncs pyproject.toml):

bash
uv init
uv add fastapi uvicorn pydantic python-dotenv google-genai google-adk
(If this step fails, remember to run glogin!)

Running the Application
Because uv is a single binary, you don't need to manually activate virtual environments:

bash
uv run python p1_v1.py
🔐 3. Security & IAM Context
BeyondCorp mTLS Override
In p1_v1.py, you will see this line:

python
os.environ["GOOGLE_API_USE_CLIENT_CERTIFICATE"] = "false"
Why? When the google-genai library tries to talk to Vertex AI from a Cloudtop, it detects the corporate network and tries to upgrade the connection to mTLS. However, because client-side SSL certs are often restricted, the mtls.py script crashes. Setting this to false forces the library to act like a normal laptop and use standard TLS.
...
Pydantic (SafetyAssessment, UserQuery):

Handles data validation without messy if statements.

Forces Gemini to return structured JSON by passing the model into output_schema.

Session Service: Stores the event history of all messages sent and received. This demo uses InMemorySessionService (lost on restart), but production apps should use VertexAiSessionService.

Runner: The manager between the API and the Agent. It fetches history from the Session Service and hands it to Gemini before every turn.

Tools: The check_robot_sensors function is provided to the agent, forcing it to fetch real-time data before executing its CRAWL/WALK/RUN reasoning.
...
git config --global pull.rebase true # Valued in Google dev to maintain linear history
