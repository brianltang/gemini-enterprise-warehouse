...
cd v1-dockerized
uv init --app
Note: uv init creates a pyproject.toml. If it generates a default hello.py, delete or rename it and ensure your code is saved in main.py.
...
bash
uv add google-adk
(If you are installing via a local wheel file in a corporate environment, use: uv pip install /path/to/google_adk.whl)
...
Once running, access the interactive API documentation here: http://localhost:8000/docs
...
(Note for Cloud Deployment: When deploying to Google Cloud Run, you do not need the .env file. Cloud Run assigns a Service Account automatically for authentication).
